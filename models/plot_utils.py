...
def simplex_plot1d(x, title=None, p_file=None, figsize=(10, 1)):
    assert np.all(x > 0)
    assert np.all(x < 1)
...
def simplex_plot2d(x, title=None, p_file=None, figsize=(8, 8)):
...
def simplex_plot(x, title=None, p_file=None, figsize=None):
    x = np.array(x)
    if len(x.shape) == 1 or (len(x.shape) == 2 and x.shape[1] == 1):
        if figsize is None:
            figsize = (10, 1)
        simplex_plot1d(x, title, p_file, figsize)
    elif len(x.shape) == 2 and x.shape[1] == 2:
        if figsize is None:
            figsize = (10, 1)
        simplex_plot1d(x[:, 0], title, p_file)
    elif len(x.shape) == 2 and x.shape[1] == 3:
        if figsize is None:
            figsize = (8, 8)
        simplex_plot2d(x, title, p_file, figsize)
    else:
