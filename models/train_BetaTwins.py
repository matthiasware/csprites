...
from Distributions.functions import kl_beta_beta, beta_params
from BTwins.transform_utils import *
from plot_utils import plot_beta_pdf
from BTwins.utils import calc_lambda


def main(config):
    p_base = Path("/mnt/experiments/csprites") / \
        Path(config["p_data"]).name / "beta_architecture"
    ts = time.time()
    st = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d_%H-%M-%S')
    config["p_experiment"] = str(p_base / "Beta[w{}_d{}]_Barlow[w{}_d{}][dkl{}_a{}_b{}]_[{}]_{}".format(
        config["beta_projector"][-1] if len(
            config["beta_projector"]) > 0 else "ID",
        len(config["beta_projector"]),
        config["barlow_projector"][-1] if len(
            config["barlow_projector"]) > 0 else "ID",
        len(config["barlow_projector"]),
        config["w_dkl"],
        config["alpha"],
        config["beta"],
        config["backbone"],
    ...
    print(Path(config.p_experiment).name)
    ...
    model = BetaTwins(
        ...
        beta_projector=config.beta_projector,
        barlow_projector=config.barlow_projector
    )
    #
    ...
    #
    print(model.beta_projector)
    print(model.barlow_projector)
    ...
    #
    print("(ALPHA, BETA) =", alpha_true, beta_true)
    #
    lmbda = calc_lambda(model.barlow_projector.dim_out)
    ...
    for epoch_idx in range(1, config.num_epochs + 1, 1):
        ...
        for (x1, x2), _ in pbar:
            ...
            # BARLOW Loss
            # #############
            z1 = model.barlow_projector(z1)
            z2 = model.barlow_projector(z2)
        ...
        if epoch_idx % config.freqs.linprob == 0 or epoch_idx == config.num_epochs:
            ...
            print(
                "    Linprob BETA @LR: {:.2f} @KNN: {:.2f}".format(linacc, knnacc))
            #
    ...
    plot_beta_pdf(dist, title="GW", p_file=p_experiment / "beta_dist_real.png", show=False)
    plt.close()
    ...
    plt.title("alphas")
    plt.savefig(p_experiment / "est_alphas.png")
    plt.close()
    #
    plt.figure(figsize=(12, 2))
    plt.bar(xx, betas_bp)
    plt.title("betas")
    plt.savefig(p_experiment / "est_betas.png")
    plt.close()

    xx = range(R_valid.shape[1])
    plt.figure(figsize=(10, 1))
    plt.bar(xx, R_valid.sum(axis=0))
    plt.title("SUM")
    plt.tight_layout()
    plt.savefig(p_experiment / "value_dist_backbone.png")
    plt.close()

    xx = range(R_valid_bp.shape[1])
    plt.figure(figsize=(10, 1))
    plt.bar(xx, R_valid_bp.sum(axis=0))
    plt.title("SUM")
    plt.tight_layout()
    plt.savefig(p_experiment / "value_dist_beta_proj.png")
    plt.close()
...
if __name__ == "__main__":
    device = '1'
    depths_beta = [1, 2]
    widths_beta = [512, 1024, 2048]
    depths_barlow = [0, 1, 2]
    width_factor_barlow = [1, 2]
    alphas_betas = [(0.1, 0.1), (0.1, 0.9)]

    step = 0
    expconfigs = []
    for d_beta in depths_beta:
        for w_beta in widths_beta:
            for d_barlow in depths_barlow:
                for w_factor in width_factor_barlow:
                    for alpha, beta in alphas_betas:
                        config = {
                            'beta_projector': [w_beta] * d_beta,
                            'barlow_projector': [w_factor * w_beta] * d_barlow,
                            'alpha': alpha,
                            'beta': beta
                        }
                        if config not in expconfigs:
                            expconfigs.append(config)
    for idx, expconfig in enumerate(expconfigs):
        print("[{}/{}] - {}".format(idx + 1, len(expconfigs), expconfig))
        config =
        {
            'device': 'cuda',
            ...
            'barlow_projector': expconfig["barlow_projector"],
            'beta_projector': expconfig["beta_projector"],
            'alpha': expconfig["alpha"],
            'beta': expconfig["beta"],
            'w_dkl': 10,
            'w_sim': 1,
            'p_ckpts': "ckpts",
